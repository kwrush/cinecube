...
const shallowStarRating = (value = 0, editable = false, hover = false) => {
  return shallow(
    <StarRating name="name" value={value} editable={editable} hover={hover} />
  );
};

const mountStarRating = (value = 0, editable = false, hover = false) => {
  return mount(
    <StarRating name="name" value={value} editable={editable}  hover={hover} />
  );
};
...
describe('<StarRating /> tests', () => {
  ...
  it('should render 3.5 stars for 3.5', () => {
    const wrapper = shallowStarRating(3.5);
    expect(wrapper.find(IoIosStar)).toHaveLength(3);
  ...
  it('should render 4.5 stars for 4.5', () => {
    const wrapper = shallowStarRating(4.5);
    expect(wrapper.find(IoIosStar)).toHaveLength(4);
    expect(wrapper.find(IoIosStarHalf)).toHaveLength(1);
  });
  ...
  it('should update state when hover', () => {
    const wrapper = mountStarRating(4, true, true);
  ...
  it('should not update state when hover if hover is false', () => {
    const wrapper = mountStarRating(4, true, false);
    wrapper.find('label').first().simulate('mouseenter');
    expect(wrapper.update().state().value).toBe(4);
  });
